...
Write-Host "⚡ Static Files & REST API Endpoints active at /api/*" -ForegroundColor Yellow
Write-Host "=======================================================" -ForegroundColor Cyan
    ...
    try {
        ...
        if ($url.StartsWith("/api/")) {
            ...
            elseif ($url -eq "/api/predictions") {
                $jsonString = '{"success":true,"data":[{"resource_type":"Electricity","prediction_type":"Excess Usage","prediction_message":"Electricity usage is 37.5% higher than normal (11.0 kWh/day).","confidence":0.92,"recommended_action":"Set AC thermostat to 24°C."},{"resource_type":"Water","prediction_type":"Anomaly","prediction_message":"Water consumption spiked by 30.8%. Possible Garden Valve leak.","confidence":0.89,"recommended_action":"Shut off Garden Valve #2."},{"resource_type":"LPG","prediction_type":"Depletion","prediction_message":"LPG at 25%. Will run out in ~4 days.","confidence":0.95,"recommended_action":"Book replacement LPG cylinder."},{"resource_type":"Food","prediction_type":"Waste","prediction_message":"Fresh Tomatoes (1.8 kg) expire in 2 days.","confidence":0.94,"recommended_action":"Cook Veggie Medley Stir-Fry."}]}'
            }
            elseif ($url -eq "/api/analysis/root-cause") {
                $jsonString = '{"success":true,"data":{"resource":"Electricity","total_excess_detected":"37.5% above baseline","primary_cause":"Air Conditioner extended operation","ranked_causes":[{"cause":"Air Conditioner Usage","confidence":"72%","impact":"10.1 kWh/day (7.2 hrs)"},{"cause":"Water Heater / Geyser","confidence":"18%","impact":"3.0 kWh/day (1.5 hrs)"},{"cause":"Smart Refrigerator","confidence":"10%","impact":"2.4 kWh/day"}],"recommended_action":"Increase AC target temperature from 20°C to 24°C."}}'
            }
            ...
            elseif ($url -eq "/api/savings") {
                $jsonString = '{"success":true,"data":{"unnecessary_electricity_cost":720,"wasted_water_cost":158,"wasted_food_cost":560,"excess_lpg_cost":412,"total_monthly_waste":1850,"potential_monthly_savings":1850,"formatted_savings":"₹1,850","co2_avoided_kg":14.5}}'
            }
            elseif ($url -eq "/api/simulator") {
                $jsonString = '{"success":true,"estimated_monthly_impact":{"electricity_saved_kwh":12.6,"water_saved_liters":210,"food_saved_kg":0.75,"money_saved_rupees":1850,"formatted_money_saved":"₹1,850","co2_avoided_kg":14.5,"new_sustainability_score":94}}'
            }
            elseif ($url -eq "/api/assistant") {
                $jsonString = '{"success":true,"reply":"✨ **SMART USAGE ALERT AI Assistant**:\n\nYour household telemetry:\n- ⚡ **Electricity**: 11.0 kWh/day (37.5% high due to AC overuse)\n- 💧 **Water**: 175 L/day (25% high due to Garden Valve leak)\n- 🔥 **LPG Reserve**: 25% (depletion in ~4 days)\n\n💡 **Grok Action Tip**: Adjusting AC to 24°C and shutting off Garden Valve #2 saves **₹1,850/month** and 14.5 kg CO₂!"}'
            }
            elseif ($url -eq "/api/benchmark") {
                $jsonString = '{"success":true,"benchmark_group":"Simulated 4-Person Eco-District Households","metrics":{"water":{"your_household":"175 L/day","neighborhood_avg":"140 L/day","comparison":"+25.0% Higher"},"electricity":{"your_household":"11.0 kWh/day","neighborhood_avg":"8.0 kWh/day","comparison":"+37.5% Higher"},"food_waste":{"your_household":"0.4 kg/week","neighborhood_avg":"1.2 kg/week","comparison":"66.7% Lower (Top Eco Performer)"}}}'
            }
            elseif ($url -eq "/api/demo/run") {
                $jsonString = '{"success":true,"scenario_name":"Hackathon Anomaly Detection & Prevention Demo","total_steps":12,"summary_impact":{"water_saved":"450 Liters/month","electricity_saved":"38 kWh/month","money_saved":"₹1,850/month","co2_avoided":"14.5 kg","sustainability_score_improvement":"+16 Points (78 → 94)"}}'
            }
            else {
